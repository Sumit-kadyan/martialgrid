...
const CommunityPage = () => {
  ...
  useEffect(() => {
    const fetchUserDataAndCommunity = async () => {
      setLoading(true);
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) { setLoading(false); return; }

      // 1. Fetch Universal Profile Data
      ...
      // 2. Safely find the user's sport based on their role
      let userSport = null;
      ...
      // 3. Fetch Relational Community Data
      if (userSport) {
        setUserProfile((prev: any) => ({ ...prev, sport: userSport }));

        const [playersRes, coachesRes, tournamentsRes] = await Promise.all([
            supabase.from('players').select('*, profiles!inner(id, name, city, avatar_url, age)').eq('sport', userSport),
            supabase.from('coaches').select('*, profiles!inner(id, name, city, avatar_url, age)').eq('sport', userSport),
            supabase.from('tournaments').select('*').eq('sport', userSport)
        ]);
      ...
      <div className="p-4 sm:p-8 max-w-7xl mx-auto">
        ...
        <header className="mb-10">
            ...
            <p className="text-xl text-muted-foreground">
               Connect with players, coaches, and tournaments in the world of <span className="font-bold text-primary capitalize">{userProfile?.sport || 'your sport'}</span>.
            </p>
        </header>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <AnimatePresence mode="popLayout">
                {/* Replaced Fragment with direct logical && rendering */}
                ...
                {!hasResults && (
                  ...
                  >
                      ...
                      <p className="text-muted-foreground max-w-md">We couldn't find anyone matching "{searchTerm}". Try searching by a different name, city, or badge.</p>
                  </motion.div>
                )}
